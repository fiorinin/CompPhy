...
class upload {
    ...
    function upload($file, $lang = 'en_GB') {
        ...
        if (empty($this->log)) {
            $this->log .= '<b>system information</b>';
            ...
            $this->log .= '-&nbsp;class version           : ' . $this->version . '';
        ...
        if (!is_array($file)) {
            if (empty($file)) {
                ...
                $this->log .= '<b>' . $this->translate("source is a local file") . ' ' . $file . '</b>';
            ...
            $this->log .= '<b>source is an uploaded file</b>';
            if ($this->uploaded) {
        ...
        if ($this->uploaded) {
            $this->log .= '<b>determining MIME type</b>';
                ...
                if ($this->mime_fileinfo) {
                    ...
                    if (function_exists('finfo_open')) {
                        if ($this->mime_fileinfo !== '') {
                            if ($this->mime_fileinfo === true) {
                                if (getenv('MAGIC') === FALSE) {
                                    ...
                                    $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;MAGIC path defaults to ' . $path . '';
                                } else {
                                    $path = getenv('MAGIC');
                        ...
                        if (is_resource($f)) {
                            ...
                            $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;Fileinfo PECL extension failed (finfo_open)';
                        }
                        ...
                        if ($f) {
                            ...
                            $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;Fileinfo PECL extension failed (finfo)';
                        }
                    } else {
                        $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;Fileinfo PECL extension not available';
                    }
                } else {
                    $this->log .= '- Fileinfo PECL extension deactivated';
                }
            }
                    ...
                    if (substr(PHP_OS, 0, 3) != 'WIN') {
                        if (function_exists('exec')) {
                            if (strlen($mime = @exec("file -bi ".escapeshellarg($this->file_src_pathname))) != 0) {
                                ...
                                $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;MIME type detected as ' . $this->file_src_mime . ' by UNIX file() command';
                                if (preg_match("/^([\.-\w]+)\/([\.-\w]+)(.*)$/i", $this->file_src_mime)) {
                                    $this->file_src_mime = preg_replace("/^([\.-\w]+)\/([\.-\w]+)(.*)$/i", '$1/$2', $this->file_src_mime);
                                    $this->log .= '-&nbsp;MIME validated as ' . $this->file_src_mime . '';
                                } else {
                                ...
                                $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;UNIX file() command failed';
                            }
                        } else {
                            $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;PHP exec() function is disabled';
                        }
                    } else {
                        $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;UNIX file() command not availabled';
                    }
                } else {
                    $this->log .= '- UNIX file() command is deactivated';
                }
            }
            ...
            if (!$this->file_src_mime || !is_string($this->file_src_mime) || empty($this->file_src_mime) || strpos($this->file_src_mime, '/') === FALSE) {
                if ($this->mime_magic) {
                    ...
                    if (function_exists('mime_content_type')) {
                        ...
                        $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;mime_content_type() is not available';
                    }
                } else {
                    $this->log .= '- mime.magic file (mime_content_type()) is deactivated';
                }
            }
                ...
                if ($this->mime_getimagesize) {
                    ...
                    if (is_array($info) && array_key_exists('mime', $info)) {
                        ...
                        if (empty($this->file_src_mime)) {
                            $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;MIME empty, guessing from type';
                    ...
                    $this->log .= '- getimagesize() is deactivated';
                }
            }
            ...
            if ($this->file_src_mime == 'application/octet-stream' || !$this->file_src_mime || !is_string($this->file_src_mime) || empty($this->file_src_mime) || strpos($this->file_src_mime, '/') === FALSE) {
                ...
                if ($this->file_src_mime == 'application/octet-stream') {
                    ...
                    $this->log .= 'MIME type set to ' . $this->file_src_mime . '';
                }
            }

            if (!$this->file_src_mime || !is_string($this->file_src_mime) || empty($this->file_src_mime) || strpos($this->file_src_mime, '/') === FALSE) {
                $this->log .= '- MIME type couldn\'t be detected! (' . (string) $this->file_src_mime . ')';
            }
            ...
            if ($this->file_is_image) {
                if ($h = fopen($this->file_src_pathname, 'r')) {
                    ...
                    if (is_array($info)) {
                        ...
                        $this->file_is_image = false;
                        $this->uploaded = false;
                        $this->log .= '- can\'t retrieve image information, image may have been tampered with';
                        $this->error = $this->translate('source_invalid');
                    }
                } else {
                    $this->log .= '- can\'t read source file directly. open_basedir restriction in place?';
                }
            }

            $this->log .= '<b>source variables</b>';
            ...
            if ($this->file_is_image) {
                $this->log .= '- source file is an image';
    ...
    function process($server_path = null) {
        ...
        if ($this->processed) {
            if (empty($server_path) || is_null($server_path)) {
                $this->log .= '<b>process file and return the content</b>';
                $return_mode = true;
            } else {
                ...
                $this->log .= '<b>process file to '  . $server_path . '</b>';
            }
        }
        ...
        if ($this->processed) {
            ...
            if ($this->no_script) {
                ...
                if ($this->file_force_extension && empty($file_src_name_ext)) {
                    if ($key = array_search($this->file_src_mime, $this->mime_types)) {
                        ...
                        $this->log .= '- file renamed as ' . $file_src_name_body . '.' . $file_src_name_ext . '!';
                    }
                }
                ...
                if ((((substr($this->file_src_mime, 0, 5) == 'text/' && $this->file_src_mime != 'text/rtf') || strpos($this->file_src_mime, 'javascript') !== false)  && (substr($file_src_name, -4) != '.txt'))
                    ...
                    $file_src_name = $file_src_name_body . '.' . $file_src_name_ext;
                    $this->log .= '- script renamed as ' . $file_src_name_body . '.' . $file_src_name_ext . '!';
                }
            }
            ...
            if ($this->file_is_image) {
                if (is_numeric($this->image_src_x) && is_numeric($this->image_src_y)) {
                    ...
                    $this->log .= '- no image properties available, can\'t enforce dimension checks : ' . $this->file_src_mime . '';
                }
            }
        }
        ...
        if ($this->processed) {
            ...
            if (!is_null($this->file_new_name_body)) { // rename file body
                ...
                $this->log .= '- new file name body : ' . $this->file_new_name_body . '';
            }
            if (!is_null($this->file_new_name_ext)) { // rename file ext
                $this->file_dst_name_ext  = $this->file_new_name_ext;
                $this->log .= '- new file name ext : ' . $this->file_new_name_ext . '';
            }
            if (!is_null($this->file_name_body_add)) { // append a string to the name
                ...
                $this->log .= '- file name body append : ' . $this->file_name_body_add . '';
            }
            ...
            if (!$return_mode) {
                if (!$this->file_auto_rename) {
                    $this->log .= '- no auto_rename if same filename exists';
                    $this->file_dst_pathname = $this->file_dst_path . $this->file_dst_name;
                } else {
                    $this->log .= '- checking for auto_rename';
                    $this->file_dst_pathname = $this->file_dst_path . $this->file_dst_name;
                    ...
                    if ($cpt>1) $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;auto_rename to ' . $this->file_dst_name . '';
                }
        ...
        if ($this->processed) {
            ...
            if (!empty($this->file_src_temp)) {
                $this->log .= '- use the temp file instead of the original file since it is a second process';
        ...
        if ($this->processed) {
            ...
            if ($image_manipulation && !@getimagesize($this->file_src_pathname)) {
                $this->log .= '- the file is not an image!';
            ...
            if ($image_manipulation) {
                ...
                $this->log .= '- image resizing or conversion wanted';
                if ($this->gdversion()) {
                ...
                if ($this->processed && $image_src) {
                    ...
                    if (!imageistruecolor($image_src)) {  // $this->image_src_type == 'gif'
                        $this->log .= '- image is detected as having a palette';
                        ...
                        $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;convert palette image to true color';
                    ...
                    if ((!empty($this->image_precrop))) {
                        ...
                        $this->log .= '- pre-crop image : ' . $ct . ' ' . $cr . ' ' . $cb . ' ' . $cl . ' ';
                    ...
                    if ($this->image_resize) {
                        ...
                        if ($this->image_ratio_x) {
                            ...
                            $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;calculate x/y size to match a number of pixels';
                            ...
                            if ((!$this->image_ratio_no_zoom_in && !$this->image_ratio_no_zoom_out)
                                ...
                                $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;doesn\'t calculate x/y sizes';
                                $this->image_dst_x = $this->image_src_x;
                                $this->image_dst_y = $this->image_src_y;
                            }
                        } else {
                            $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;use plain sizes';
                            $this->image_dst_x = $this->image_x;
                            $this->image_dst_y = $this->image_y;
                        }
                        ...
                        $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;resized image object created';
                    ...
                    if ((!empty($this->image_crop) || !is_null($ratio_crop))) {
                        ...
                        $this->log .= '- crop image : ' . $ct . ' ' . $cr . ' ' . $cb . ' ' . $cl . ' ';
                    ...
                    if ($gd_version >= 2 && is_numeric($this->image_rotate)) {
                        ...
                        if ($this->image_rotate != 0) {
                            ...
                            $this->log .= '- rotate image : ' . $this->image_rotate . '';
                            for ($x = 0; $x < $this->image_dst_x; $x++) {
                                for ($y = 0; $y < $this->image_dst_y; $y++){
                    ...
                    if ($gd_version >= 2 && ($this->image_negative || $this->image_greyscale || is_numeric($this->image_threshold)|| is_numeric($this->image_brightness) || is_numeric($this->image_contrast) || !empty($this->image_tint_color))) {
                        $this->log .= '- apply tint, light, contrast correction, negative, greyscale and threshold';
                    ...
                    if ($gd_version >= 2 && !empty($this->image_border)) {
                        ...
                        $this->log .= '- add border : ' . $ct . ' ' . $cr . ' ' . $cb . ' ' . $cl . '';
                        $this->image_dst_x = $this->image_dst_x + $cl + $cr;
                        $this->image_dst_y = $this->image_dst_y + $ct + $cb;
                    ...
                    if ($gd_version >= 2 && !empty($this->image_border_transparent)) {
                        ...
                        $this->log .= '- add transparent border : ' . $ct . ' ' . $cr . ' ' . $cb . ' ' . $cl . '';
                        // we now create an image, that we fill with the border color                                                                           
                        $tmp = $this->imagecreatenew($this->image_dst_x, $this->image_dst_y);                                                                   
                    ...
                    if ($gd_version >= 2 && is_numeric($this->image_frame)) {
                        if (is_array($this->image_frame_colors)) {
                            ...
                            $this->log .= '- add frame : ' . implode(' ', $this->image_frame_colors) . '';
                        } else {
                            $this->log .= '- add frame : ' . $this->image_frame_colors . '';
                        ...
                        if ($watermark_type == IMAGETYPE_GIF) {
                            if (!function_exists('imagecreatefromgif')) {
                                ...
                                if (!$filter) {
                                    ...
                                    $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;watermark source image is GIF';
                            ...
                            if (!function_exists('imagecreatefromjpeg')) {
                                ...
                                if (!$filter) {
                                    ...
                                    $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;watermark source image is JPEG';
                                    $watermark_checked = true;
                                }
                            }
                            ...
                            if (!function_exists('imagecreatefrompng')) {
                                ...
                                if (!$filter) {
                                    ...
                                    $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;watermark source image is PNG';
                                    $watermark_checked = true;
                                }
                            }
                            ...
                            if (!method_exists($this, 'imagecreatefrombmp')) {
                                ...
                                if (!$filter) {
                                    ...
                                    $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;watermark source image is BMP';
                                    $watermark_checked = true;
                                }
                            }
                        } else {
                    ...
                    if ($this->image_reflection_height) {
                        $this->log .= '- add reflection : ' . $this->image_reflection_height . '';
                    ...
                    if (is_numeric($this->jpeg_size) && $this->jpeg_size > 0 && ($this->image_convert == 'jpeg' || $this->image_convert == 'jpg')) {
                        ...
                        $this->log .= '- JPEG desired file size : ' . $this->jpeg_size . '';
                        ...
                        $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;JPEG quality factor set to ' . $this->jpeg_quality . '';
                    }
                    ...
                    switch($this->image_convert) {
                        case 'gif':
                            ...
                            if (imageistruecolor($image_dst)) {
                                $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;true color to palette';
                        ...
                        case 'bmp':
                            ...
                            $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;fills in transparency with default color';
                            list($red, $green, $blue) = $this->getcolors($this->image_default_color);
                ...
                $this->log .= '- no image processing wanted';
        ...
        if ($this->processed) {
            $this->log .= '- <b>process OK</b>';
        } else {
            $this->log .= '- <b>error</b>: ' . $this->error . '';
        }
